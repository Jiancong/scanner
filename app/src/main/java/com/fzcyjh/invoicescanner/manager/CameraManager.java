...
public class CameraManager {
    ...
    private String cameraSizeListToString(List<Size> lstSize){
        String  strRet = "";
        int nIndex = 0;
        for(Camera.Size size : lstSize){
            nIndex++;
            strRet = strRet + "[ID:" + nIndex + ",w:" + size.width + ", h:" + size.height + "],";
        }
        return strRet;
    }
    /**
    ...
    public Camera open(int cameraId, Context mContext) {
        ...
        //Log.d(TAG, "mSupportedPreviewSizes:" + cameraSizeListToString(mSupportedPreviewSizes));
        List<Size> lstPictureSupportedSizes = camera.getParameters().getSupportedPictureSizes();
        Log.d(TAG, "mSupportedPreviewSizes:" + cameraSizeListToString(mSupportedPreviewSizes));
        Log.d(TAG, "mSupportedPictureSizes:" + cameraSizeListToString(lstPictureSupportedSizes));

        if (mSupportedPreviewSizes != null) {
            mPreviewSize = getOptimalPreviewSize(
                    mSupportedPreviewSizes, getScreenWidth(mContext), getScreenHeight(mContext));
        }
        Log.d(TAG, "preview w=" + mPreviewSize.width + ",h=" + mPreviewSize.height);

        mPictureSize = mPreviewSize;
        if (lstPictureSupportedSizes != null) {
            // 4k
            mPictureSize = getOptimalPreviewSize(
                    lstPictureSupportedSizes, getScreenWidth(mContext) * 2, getScreenHeight(mContext) * 2 );
        }
        Log.d(TAG, "picture w=" + mPictureSize.width + ", h=" + mPictureSize.height);
