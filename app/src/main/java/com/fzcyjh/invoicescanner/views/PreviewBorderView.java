package com.fzcyjh.invoicescanner.views;
...
public class PreviewBorderView extends SurfaceView implements SurfaceHolder.Callback, Runnable {
    private static final String DEFAULT_TIPS_TEXT = "请将发票置于框内扫描，并尽量对齐边框";
    ...
    private float mInvoiceW = 660;
    private float mInvoiceH = 390;
    /**
     * 自定义属性
     */
    ...
    private void init() {
        ...
        this.mPaintLine.setColor(Color.argb(255, 52, 152, 255));
...
//            this.mScreenW = (this.mScreenH * 4 / 3);
            ...
            RectF rtfRoi = calcRoiRect(this.mScreenW, this.mScreenH);
            DrawOci(rtfRoi);
            ...
            this.mCanvas.drawText(tipText, rtfRoi.left + (rtfRoi.width() - length) / 2, this.mScreenH * 1 / 2 - tipTextSize, mPaintLine);
            Log.d("TAG", "left:" + (this.mScreenW / 2 - this.mScreenH * 2 / 3 + this.mScreenH * 1 / 6));
            Log.d("TAG", "top:" + (this.mScreenH * 1 / 6));
            Log.d("TAG", "right:" + (this.mScreenW / 2 + this.mScreenH * 2 / 3 - this.mScreenH * 1 / 6));
            Log.d("TAG", "bottom:" + (this.mScreenH - this.mScreenH * 1 / 6));
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    static public   RectF calcRoiRect(float fScreenW, float fScreenH){
        Log.d(TAG, "w:" + fScreenW + ", H:" + fScreenH);
        float fInvoiceW = 66;
        float fInvoiceH = 39;
        float fUnit = fInvoiceW / fInvoiceH;
        float fBorderReservedH = fScreenH * 1 / 8;
        float fRoiHeight = fScreenH - fBorderReservedH * 2;
        float fRoiWidth = fRoiHeight * fUnit;
        PointF ptfRoiCenterPoint = new PointF((fScreenW - (fScreenW / 6))/ 2, fScreenH / 2);
        return new RectF(ptfRoiCenterPoint.x - fRoiWidth / 2, ptfRoiCenterPoint.y - fRoiHeight / 2,
                ptfRoiCenterPoint.x + fRoiWidth / 2, ptfRoiCenterPoint.y + fRoiHeight / 2);
    }

    private void DrawOci(RectF rtfRoi){
        ...
        this.mCanvas.drawRect(rtfRoi, this.mPaint);
        int nBorderLenght = (int) (rtfRoi.height() / 6);
        // top left
        this.mCanvas.drawLine(rtfRoi.left, rtfRoi.top, rtfRoi.left + nBorderLenght, rtfRoi.top, this.mPaintLine);
        this.mCanvas.drawLine(rtfRoi.left, rtfRoi.top, rtfRoi.left, rtfRoi.top + nBorderLenght, this.mPaintLine);
        // top right
        this.mCanvas.drawLine(rtfRoi.right, rtfRoi.top, rtfRoi.right - nBorderLenght, rtfRoi.top, this.mPaintLine);
        this.mCanvas.drawLine(rtfRoi.right, rtfRoi.top, rtfRoi.right, rtfRoi.top + nBorderLenght, this.mPaintLine);
        // botton left
        this.mCanvas.drawLine(rtfRoi.left, rtfRoi.bottom, rtfRoi.left + nBorderLenght, rtfRoi.bottom, this.mPaintLine);
        this.mCanvas.drawLine(rtfRoi.left, rtfRoi.bottom, rtfRoi.left, rtfRoi.bottom - nBorderLenght, this.mPaintLine);

        // bottom right
        this.mCanvas.drawLine(rtfRoi.right, rtfRoi.bottom, rtfRoi.right - nBorderLenght, rtfRoi.bottom, this.mPaintLine);
        this.mCanvas.drawLine(rtfRoi.right, rtfRoi.bottom, rtfRoi.right, rtfRoi.bottom - nBorderLenght, this.mPaintLine);
        /*
